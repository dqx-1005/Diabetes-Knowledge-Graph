...
from Detect.conlleval import return_report
...
def result_to_json(string, tags):
    ...
    for char, tag in zip(string, tags):
        if tag[0] == "B":
            ...
            entity_name += char
            if idx == len(tags) - 1:
                item["entities"].append({"word": entity_name, "start": entity_start, "end": idx, "type": tag_ent})
        elif tag[0] == "O":
            if len(entity_name) > 0:
                if entity_name not in unique_name:
                    unique_name.append(entity_name)
                    if len(tag_ent) >0:
                        item["entities"].append({"word": entity_name, "start": entity_start, "end": idx, "type": tag_ent})
            entity_name = ""
        idx += 1
